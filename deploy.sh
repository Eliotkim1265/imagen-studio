...
# If DATABASE_URL_ENV_VAR_VALUE is still empty or the placeholder, prompt for manual input
if [ -z "$DATABASE_URL_ENV_VAR_VALUE" ] || [ "$DATABASE_URL_ENV_VAR_VALUE" == "YOUR_MANUAL_DATABASE_URL_HERE" ]; then
    echo ""
    echo "You need to provide the DATABASE_URL for your existing database."
    echo "Example format for Cloud SQL: postgresql://USER:PASSWORD@/DB_NAME?host=/cloudsql/PROJECT_ID:REGION:INSTANCE_NAME"
    echo "Example for other PostgreSQL: postgresql://USER:PASSWORD@HOST_ADDRESS:PORT/DB_NAME"
    read -p "Please enter the full DATABASE_URL: " MANUAL_DATABASE_URL
    if [ -n "$MANUAL_DATABASE_URL" ]; then
        DATABASE_URL_ENV_VAR_VALUE="$MANUAL_DATABASE_URL"
        echo "Using manually provided DATABASE_URL: $DATABASE_URL_ENV_VAR_VALUE"
    else
        echo "WARNING: No DATABASE_URL provided. The application might not connect to a database."
        DATABASE_URL_ENV_VAR_VALUE="YOUR_MANUAL_DATABASE_URL_HERE" # Reset to placeholder if still nothing
    fi
fi

# Grant Cloud Run SA access to Cloud SQL instance (only if an instance connection name was determined)
if [ -n "$INSTANCE_CONNECTION_NAME_ENV_VAR_VALUE" ]; then
    echo "Granting Cloud Run SA (${CR_SA_EMAIL}) 'Cloud SQL Client' role..."
    gcloud projects add-iam-policy-binding "$GCP_PROJECT_ID" \
        --member="serviceAccount:${CR_SA_EMAIL}" \
        --role="roles/cloudsql.client" --condition=None
    echo "  - Cloud Run SA granted 'Cloud SQL Client'."
else
    echo "Skipping 'Cloud SQL Client' role grant as Cloud SQL instance connection name was not set up by this script."
fi
...
SQL_INSTANCE_FLAG_DEPLOY="" # Initialize
...
echo "--- DEBUG: Deploy Command Variables ---"
echo "CLOUD_RUN_SERVICE_NAME: ${CLOUD_RUN_SERVICE_NAME}"
echo "IMAGE_TAG_URL: ${IMAGE_TAG_URL}"
echo "GCP_REGION: ${GCP_REGION}"
echo "CR_SA_EMAIL: ${CR_SA_EMAIL}"
echo "SECRET_MAPPINGS_CLOUDRUN: ${SECRET_MAPPINGS_CLOUDRUN}"
echo "ENV_VARS_CLOUDRUN: ${ENV_VARS_CLOUDRUN}"
echo "SQL_INSTANCE_FLAG_DEPLOY: ${SQL_INSTANCE_FLAG_DEPLOY}" # Check if this is empty or has a value
